...
var WindowManage = function() {
    ...
    this.cwPoint = {x: 595, y: 55};//属性板初始位置

    this.listWindow = null;//列表板
    this.lwPoint = {x: 0, y: 55};//列表板初始位置
}
WindowManage.prototype = {
    ...
    structNameChange : function(){
        if (this.changeFlag)
            ...
            var no = Struct.structManage.searchByID(this.useID);
            //纠正错误输入
            ...
            Struct.structures[no].data.name = data;
            Struct.cooperationManage.sendInfo("Modify", Struct.structures[no].id);
            this.updateLW();
        }
    },
    ...
    structQualityChange : function(){
        if (this.changeFlag)
            ...
            if (data != '-') {
                ...
                Struct.structures[no].data.quality = parseFloat(data);
                Struct.cooperationManage.sendInfo("Modify", Struct.structures[no].id);
            }
        }
    },
    ...
    powerConsumptionChange : function(){
        if (this.changeFlag)
            ...
            if (data != '-') {
                ...
                Struct.structures[no].data.consumption = parseFloat(data);
                Struct.cooperationManage.sendInfo("Modify", Struct.structures[no].id);
            }
        }
    },
    ...
    positionChange : function(){
        if (this.changeFlag)
            ...
            if (data.x != '-' && data.y != '-' && data.z != '-') {
                Struct.structures[no].refresh(new THREE.Vector3(data.x, data.y, data.z), null, null);
                Struct.render();
                Struct.cooperationManage.sendInfo("Modify", Struct.structures[no].id);
            }
        }
    ...
    rotationChange : function(){
        if (this.changeFlag)
            ...
            if (data.x != '-' && data.y != '-' && data.z != '-') {
                Struct.structures[no].refresh(null, new THREE.Vector3(data.x * Math.PI / 180, data.y * Math.PI / 180, data.z * Math.PI / 180), null);
                Struct.render();
                Struct.cooperationManage.sendInfo("Modify", Struct.structures[no].id);
            }
        }
    ...
    sizeChange : function(){
        if (this.changeFlag)
            ...
            var initSize = Struct.structures[no].data.initSize;
            //纠正错误输入
            var data = {
