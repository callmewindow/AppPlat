...
SceneManage.prototype = {
    ...
    resetScene: function () {
        ...
        Struct.structManage.deleteByID(idList);
        Struct.structures = new Array();
        Struct.render();
    },
    ...
    //更新视角
    updateView : function(position, rotation){
        Struct.camera.position.set(position.x, position.y, position.z);
        Struct.camera.rotation.set(rotation.x, rotation.y, rotation.z);
        Struct.light.position.addVectors(Struct.camera.position, new THREE.Vector3(0, 0, 0));
        //恢复视角控制器参数
        this.orbitControl.target = new THREE.Vector3();
        this.orbitControl.center = this.orbitControl.target;
    ...
    drawGrid : function(){
        ...
        this.gridHelper.position = new THREE.Vector3( 0, -50, 0 );
    ...
    addOrbitControl : function(){
        ...
        this.orbitControl.addEventListener( 'change', function(event){
            ...
            if (temp.x * temp.x + temp.y * temp.y + temp.z * temp.z > 3 *15000*15000) Struct.sceneManage.initView();
        } );
    },
    ...
    addSkyBox : function(){
        ...
        Struct.scene.add( new THREE.Mesh( skyGeometry, new THREE.MeshFaceMaterial( materialArray ) ) );
        Struct.renderer.clear();
        Struct.render();
    }
}