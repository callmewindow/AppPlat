...
StructManage.prototype = {
    ...
    addMouseControl : function(){
        ...
        Struct.renderer.domElement.ondblclick = function(event){
            ...
            if (Struct.structManage.assemblyFlag == -1){
                ...
                //中断吸附操作
                Ext.MessageBox.show({title: "操作提示",msg: "吸附操作取消",buttons: Ext.Msg.OK});
    ...
    addTransformControl : function(){
        ...
        this.transformControl.addEventListener( 'change',function(){
            ...
            //防止越界操作
            var safeTest = Struct.structures[Struct.structManage.searchByID(Struct.windowManage.useID)].object;
            if (safeTest.position.x > 1000) {safeTest.position.x = 1000;Struct.structManage.transformControl.attach(safeTest);}
            if (safeTest.position.y > 1000) {safeTest.position.y = 1000;Struct.structManage.transformControl.attach(safeTest);}
            if (safeTest.position.z > 1000) {safeTest.position.z = 1000;Struct.structManage.transformControl.attach(safeTest);}
            if (safeTest.position.x < -1000) {safeTest.position.x = -1000;Struct.structManage.transformControl.attach(safeTest);}
            if (safeTest.position.y < -1000) {safeTest.position.y = -1000;Struct.structManage.transformControl.attach(safeTest);}
            if (safeTest.position.z < -1000) {safeTest.position.z = -1000;Struct.structManage.transformControl.attach(safeTest);}
    ...
    alignmentX: function(){
        ...
        for (var i = 0; i < Struct.structures.length; i++) {
            if (Struct.structures[i].selectFlag){
//                Struct.structures[i].refresh(new THREE.Vector3(Struct.structures[i].object.position.x, ty, tz));
                Struct.transform(Struct.structures[i].object, new THREE.Vector3(Struct.structures[i].object.position.x, ty, tz), Struct.structures[i].object.rotation, 500);
            }
        }
        Struct.render();
        setTimeout(function () {
    ...
    alignmentY: function(){
        ...
        for (var i = 0; i < Struct.structures.length; i++) {
            if (Struct.structures[i].selectFlag){
//                Struct.structures[i].refresh(new THREE.Vector3(tx, Struct.structures[i].object.position.y, tz));
    ...
    alignmentZ: function(){
        ...
        for (var i = 0; i < Struct.structures.length; i++) {
            if (Struct.structures[i].selectFlag){
//                Struct.structures[i].refresh(new THREE.Vector3(tx, ty, Struct.structures[i].object.position.z));
                Struct.transform(Struct.structures[i].object, new THREE.Vector3(tx, ty, Struct.structures[i].object.position.z), Struct.structures[i].object.rotation, 500);
            }
        }
        Struct.render();
        setTimeout(function () {
    ...
    combinaOperate : function(){
        ...
        for (var i = 0; i < idList.length; i++){
            this.breakByID(Struct.structures[this.searchByID(idList[i])].id);
    ...
    combineByID : function(idList, id){
        ...
        if (id != null) Struct.structures[total].id = id;
        Struct.structures[total].initByCombina(idList);
    ...
    breakOperate : function(){
        ...
        for (var i = 0; i < Struct.structures.length; i++) {
            if (Struct.structures[i].selectFlag) {
                if (id != null){
                    Ext.MessageBox.show({title: "操作提示",msg: "请仅选中一个组合体",buttons: Ext.Msg.OK});
                    return;
                }
                else
        ...
        if (id == null){
            Ext.MessageBox.show({title: "操作提示",msg: "请至少选中一个组合体",buttons: Ext.Msg.OK});
            return;
        }
        //选中的不是组合体也不行
        var no = Struct.structManage.searchByID(id);
        if (!Struct.structures[no].combinaFlag){
            Ext.MessageBox.show({title: "操作提示",msg: "请选中一个组合体以进行分离",buttons: Ext.Msg.OK});
            return;
        }
    ...
    breakByID : function(id){
        var no = this.searchByID(id);
        var idList = Struct.structures[no].idList;
        //重新建立子组件
        for (var i = 0; i < Struct.structures[no].object.children.length; i++){
            var total = Struct.structures.length;
            Struct.structures[total] = new Structure();
            Struct.structures[total].id = idList[i];
            Struct.structures[total].object.add(Struct.structures[no].object.children[i].children[0]);
            //设置每个分解后组件的三维信息
//            Struct.structures[total].object.position.x = Struct.structures[no].object.children[i].position.x * Struct.structures[no].object.scale.x;
//            Struct.structures[total].object.position.y = Struct.structures[no].object.children[i].position.y * Struct.structures[no].object.scale.y;
//            Struct.structures[total].object.position.z = Struct.structures[no].object.children[i].position.z * Struct.structures[no].object.scale.z;
//            Struct.structures[total].object.position.applyMatrix4(Matrixs.axisZRotationMatrix(Struct.structures[no].object.rotation.z));
//            Struct.structures[total].object.position.applyMatrix4(Matrixs.axisYRotationMatrix(Struct.structures[no].object.rotation.y));
//            Struct.structures[total].object.position.applyMatrix4(Matrixs.axisXRotationMatrix(Struct.structures[no].object.rotation.x));
//            Struct.structures[total].object.position.add(Struct.structures[no].object.position);
//            //Todo 分解后旋转方向无法恢复
//            var temp = Struct.structures[no].object.children[i].rotation.x + Struct.structures[no].object.rotation.x;
//            if (temp > Math.PI) temp -= (2 * Math.PI);
//            if (temp < -Math.PI) temp += (2 * Math.PI);
//            Struct.structures[total].object.rotation.x = temp;
//            temp = Struct.structures[no].object.children[i].rotation.y + Struct.structures[no].object.rotation.y;
//            if (temp > Math.PI) temp -= (2 * Math.PI);
//            if (temp < -Math.PI) temp += (2 * Math.PI);
//            Struct.structures[total].object.rotation.y = temp;
//            temp = Struct.structures[no].object.children[i].rotation.z + Struct.structures[no].object.rotation.z;
//            if (temp > Math.PI) temp -= (2 * Math.PI);
//            if (temp < -Math.PI) temp += (2 * Math.PI);
//            Struct.structures[total].object.rotation.z = temp;
//            Struct.structures[total].object.scale.x = Struct.structures[no].object.children[i].scale.x * Struct.structures[no].object.scale.x;
//            Struct.structures[total].object.scale.y = Struct.structures[no].object.children[i].scale.y * Struct.structures[no].object.scale.y;
//            Struct.structures[total].object.scale.z = Struct.structures[no].object.children[i].scale.z * Struct.structures[no].object.scale.z;
            Struct.structures[total].object.applyMatrix(Struct.structures[no].object.children[i].matrixWorld);
            Struct.scene.add(Struct.structures[total].object);
            Struct.structures[total].pathobj = Struct.structures[no].pathobj[i];
            Struct.structures[total].pathmtl = Struct.structures[no].pathmtl[i];
            //基础模型和非基础模型的包络盒不一样
            if (Struct.structures[no].basicFlag[i]) {
                Struct.structures[total].boxHelper = new THREE.BoxHelper(Struct.structures[total].object.children[0]);
                Struct.structures[total].basicFlag = true;
            }else {
                Struct.structures[total].boxHelper = new THREE.BoxHelper(
                    Struct.structures[total].object.children[0].children[Struct.structures[total].object.children[0].children.length - 1]);
                Struct.structures[total].basicFlag = false;
            }
            //            //注释框
//            Struct.structures[total].remark.style.zIndex = 3;
//            Struct.structures[total].remark.style.position = "fixed";
//            Struct.structures[total].remark.style.display = "none";
//            Struct.container.parentNode.appendChild(Struct.structures[total].remark);
            //附加数据
            Struct.structures[total].data.name = Struct.structures[no].data.nameList[i];
            Struct.structures[total].data.quality = Struct.structures[no].data.qualityList[i];
            Struct.structures[total].data.consumption = Struct.structures[no].data.consumptionList[i];
            Struct.structures[total].data.initSize = Struct.structures[no].data.initSizeList[i];
    ...
    assemblyAction : function(intersect) {
        ...
        if (this.assemblyFlag == 1) {
            ...
            if (isNaN(safeTest.position.x) || isNaN(safeTest.position.y) || isNaN(safeTest.position.z) || this.assemblyObjectA.No == this.assemblyObjectB.No){
                Ext.MessageBox.show({title: "操作提示", msg: "吸附失败，请重试", buttons: Ext.Msg.OK});
            ...
            Struct.transform(Struct.structures[this.assemblyObjectB.No].object,
                ...
                nRotation, 500);
//            Struct.structures[this.assemblyObjectB.No].object.position.x -= this.assemblyObjectB.Position.x - this.assemblyObjectA.Position.x;
//            Struct.structures[this.assemblyObjectB.No].object.position.y -= this.assemblyObjectB.Position.y - this.assemblyObjectA.Position.y;
//            Struct.structures[this.assemblyObjectB.No].object.position.z -= this.assemblyObjectB.Position.z - this.assemblyObjectA.Position.z;
    ...
    structCopy : function(){
        ...
        //将选中的组件复制
        for (var i = 0; i < Struct.structures.length; i++){
            if (!Struct.structures[i].selectFlag) continue;
            if (!Struct.structures[i].combinaFlag) {
                var total = Struct.structures.length;
                Struct.structures[total] = new Structure();
                if (Struct.structures[i].basicFlag)
                    Struct.structures[total].initBasicModel(Struct.structures[i].pathobj,
                        new THREE.Vector3(Struct.structures[i].object.position.x,
                                Struct.structures[i].object.position.y,
                                Struct.structures[i].object.position.z));
                else
                    Struct.structures[total].initByOBJMTL(total, Struct.structures[i].pathobj, Struct.structures[i].pathmtl,
                        new THREE.Vector3(Struct.structures[i].object.position.x,
                                           Struct.structures[i].object.position.y,
                                           Struct.structures[i].object.position.z));
                Struct.render();
                dataCopy(Struct.structures[total].object.rotation, Struct.structures[i].object.rotation);
                dataCopy(Struct.structures[total].object.scale, Struct.structures[i].object.scale);
                Struct.structures[total].data.name = Struct.structures[i].data.name + '(2)';
                Struct.structures[total].data.quality = Struct.structures[i].data.quality;
                Struct.structures[total].data.consumption = Struct.structures[i].data.consumption;
                Struct.structures[total].data.initSize = Struct.structures[i].data.initSize;
                Struct.structures[total].object.position.x += 50;
                Struct.structures[total].object.position.y += 50;
                Struct.structures[total].object.position.z += 50;
                Struct.cooperationManage.sendInfo("Addition", Struct.structures[Struct.structures.length - 1].id);
                Struct.structures[total].object.position.x -= 50;
                Struct.structures[total].object.position.y -= 50;
                Struct.structures[total].object.position.z -= 50;
                //动画效果
                Struct.transform(Struct.structures[total].object,
                    new THREE.Vector3(Struct.structures[total].object.position.x + 50, Struct.structures[total].object.position.y + 50, Struct.structures[total].object.position.z + 50),
                    Struct.structures[total].object.rotation, 300);
            }else{
                var total = Struct.structures.length;
                Struct.structures[total] = new Structure();
                //复制组件信息
                Struct.structures[total].object = Struct.structures[i].object.clone();
                Struct.scene.add(Struct.structures[total].object);
                Struct.render();
                Struct.structures[total].idList = new Array();
                for (var j = 0; j < Struct.structures[i].idList.length; j++)
                    Struct.structures[total].idList[j] = Math.floor(Math.random() * 8999999999999999 + 1000000000000000);
                Struct.structures[total].pathobj = Struct.structures[i].pathobj;
                Struct.structures[total].pathmtl = Struct.structures[i].pathmtl;
                Struct.structures[total].basicFlag = Struct.structures[i].basicFlag;
                Struct.structures[total].boxHelper = new Array();
                for (var j = 0; j < Struct.structures[total].object.children.length; j++) {
                    if (Struct.structures[total].basicFlag[j])
                        Struct.structures[total].boxHelper[j] = new THREE.BoxHelper(Struct.structures[total].object.children[j].children[0]);
                    else
                        Struct.structures[total].boxHelper[j] = new THREE.BoxHelper(
                            Struct.structures[total].object.children[j].children[0].children[Struct.structures[total].object.children[j].children[0].children.length - 1]);
                    Struct.render();
                }
                Struct.structures[total].combinaFlag = true;
//                //设置注释框
//                Struct.structures[total].remark.style.zIndex = 3;
//                Struct.structures[total].remark.style.position = "fixed";
//                Struct.structures[total].remark.style.display = "none";
//                Struct.container.parentNode.appendChild(Struct.structures[total].remark);
                ...
                Struct.structures[total].data.initSize = Struct.structures[i].data.initSize;
                for (var j = 0; j < Struct.structures[i].idList.length; j++) {
                    Struct.structures[total].data.nameList[j] = Struct.structures[i].data.nameList[j];
                    Struct.structures[total].data.qualityList[j] = Struct.structures[i].data.qualityList[j];
                    Struct.structures[total].data.consumptionList[j] = Struct.structures[i].data.consumptionList[j];
                    Struct.structures[total].data.initSizeList[j] = Struct.structures[i].data.initSizeList[j];
                }
                //复制体的三维信息
                Struct.structures[total].object.position.x = Struct.structures[i].object.position.x;
                Struct.structures[total].object.position.y = Struct.structures[i].object.position.y;
                Struct.structures[total].object.position.z = Struct.structures[i].object.position.z;
                dataCopy(Struct.structures[total].object.rotation, Struct.structures[i].object.rotation);
                dataCopy(Struct.structures[total].object.scale, Struct.structures[i].object.scale);
                //协同添加组合体
                var dataInfo = new Array();
                for (var j = 0; j < Struct.structures[total].idList.length; j++){
                    dataInfo[dataInfo.length] = {
                        id: Struct.structures[total].idList[j],
                        basicFlag: Struct.structures[total].basicFlag[j],
                        pathobj: Struct.structures[total].pathobj[j],
                        pathmtl: Struct.structures[total].pathmtl[j],
                        position: {
                            x: Struct.structures[total].object.position.x + Struct.structures[total].object.children[j].position.x + 50,
                            y: Struct.structures[total].object.position.y + Struct.structures[total].object.children[j].position.y + 50,
                            z: Struct.structures[total].object.position.z + Struct.structures[total].object.children[j].position.z + 50
                        },
                        rotation: {
                            x: Struct.structures[total].object.children[j].rotation.x,
                            y: Struct.structures[total].object.children[j].rotation.y,
                            z: Struct.structures[total].object.children[j].rotation.z
                        },
                        scale: {
                            x: Struct.structures[total].object.children[j].scale.x * Struct.structures[total].object.scale.x,
                            y: Struct.structures[total].object.children[j].scale.y * Struct.structures[total].object.scale.y,
                            z: Struct.structures[total].object.children[j].scale.z * Struct.structures[total].object.scale.z
                        },
                        data: Struct.structures[total].data
                    }
                }
                Struct.cooperationManage.sendInfo("AddandCmb", Struct.structures[total].id, dataInfo);
                //动画效果
                Struct.transform(Struct.structures[total].object,
                    new THREE.Vector3(Struct.structures[total].object.position.x + 50, Struct.structures[total].object.position.y + 50, Struct.structures[total].object.position.z + 50),
                    Struct.structures[total].object.rotation, 300);
    ...
    deleteByID : function(id, flag){
        for (var i = 0; i < id.length; i++) {
            var no = Struct.structManage.searchByID(id[i]);
            Struct.structures[no].delete();
            Struct.structures.splice(no,1);
    ...
    //删除所有物体
    deleteAll : function(flag){
        for (var i = 0; i < Struct.structures.length; i++) {
            if (flag == null) Struct.cooperationManage.sendInfo("Delete", Struct.structures[i].id);
            var no = Struct.structManage.searchByID(Struct.structures[i].id);
            Struct.structures[no].delete();
            Struct.structures.splice(no,1);
            i--;
        }
    ...
    //将指定ID选中
    selectById : function(idList){
        for (var i = 0; i < idList.length; i++)
            Struct.structures[this.searchByID(idList[i])].select();
        Struct.render();
        Struct.windowManage.updateLW();
    },
    ...
    unselectById : function(idList){
        for (var i = 0; i < idList.length; i++)
            Struct.structures[this.searchByID(idList[i])].unselect();
        Struct.render();
        Struct.windowManage.updateLW();
    },
    ...
    lockById : function(idList){
        for (var i = 0; i < idList.length; i++)
            Struct.structures[this.searchByID(idList[i])].lock();
        Struct.render();
        Struct.windowManage.updateLW();
    },
    ...
    mousePoint : function(intersects){
        ...
        for (var i = 0; i < intersects.length; i++) {
            ...
            for (var j = 0; j < Struct.structures.length; j++) {
                if (flag) break;
                if (Struct.structures[j].lockFlag) continue;
                //扫描所有可能的id
                if (!Struct.structures[j].combinaFlag) {
                    if (Struct.structures[j].object.children[0].id == intersectID)
                        flag = true;
                    else
                        for (var k = 0; k < Struct.structures[j].object.children[0].children.length; k++)
                            if (Struct.structures[j].object.children[0].children[k].id == intersectID) {
                                flag = true;
                                break;
                            }
                }else {
                    for (var l = 0; l < Struct.structures[j].object.children.length; l++) {
                        if (Struct.structures[j].object.children[l].children[0].id == intersectID) {
                            flag = true;
                            break;
                        }else
                            for (var k = 0; k < Struct.structures[j].object.children[l].children[0].children.length; k++)
                                if (Struct.structures[j].object.children[l].children[0].children[k].id == intersectID) {
                                    flag = true;
                                    break;
                                }
                        if (flag) break;
...
//                        Ext.MessageBox.show({title: "操作提示", msg: "（2/2）请单击被吸附物体某一面", buttons: Ext.Msg.OK});
                        ...
                        if (Struct.structures[j].combinaFlag && Struct.structures[j].selectFlag){
                            ...
                            Struct.structures[j].select();
                            Struct.cooperationManage.sendInfo("LockControl", Struct.structures[j].id, true);
                            Struct.windowManage.updateLW();
                        }
                    }
                    flag = true;
