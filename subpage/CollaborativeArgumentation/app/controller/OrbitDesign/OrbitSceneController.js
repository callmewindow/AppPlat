 ...
 */
Ext.define('AppPlat.controller.OrbitDesign.OrbitSceneController', {
    ...
    views: [
        'SolutionTree',
        'AppPlat.view.OrbitDesign.VisibilityWindow'
    ],
    refs: [
        {
            ref: 'solutionTree',
...
//            },
            ...
            //设置可见性的响应函数
            "buttongrouppanel checkboxgroup[itemId=visibilityCheckboxGroup]":{
...
//            }
    ...
    btnSaveOrbitFileClick: function () {
        ...
        me.getButtongrouppanel().queryById('OrbitMenu').menu.hide();
        if (rootNode == null) {
            Ext.MessageBox.alert('操作提示', '您还未添加或打开文件', function (btn) {
            });
            return;
        }
//        var fileName = me.getApplication().getController('TreeController').fileName;
        ...
        if (fileName != null) {
            ...
            me.getApplication().getController('AppPlat.controller.TreeController').isFileSaved = true ;
        }
    },
    //导出场景文件
    ...
    btnOpenOrbitFileClick: function () {
        ...
        if(this.orbitFileWindow == null){
            this.orbitFileWindow = Ext.create('AppPlat.view.OrbitDesign.OrbitFileWindow');
        }
        ...
        if(me.getSolutionTree().getRootNode() != null){
            Ext.MessageBox.confirm('提示', '未保存的工作将会丢失，是否继续？', function(btn) {
                if (btn == 'yes') {
                    ...
                    me.openOrbitTaskRequest(record);
                }
            });
        }
        else{
     ...
     * @param record
     */
    openOrbitTaskRequest: function(record){
        var me = this;
        if (record != 'undefined') {
            var perTaskId = null;
            Ext.Ajax.request({
                url: APC.APPPLAT_SERVICE_URL_PREFIX + "TaskManage/getPersonalTaskInfo.json",
                method: 'GET',
                params: {
                    perTaskName: record[0].data.name
                },
                success: function (response) {
                    if (response.responseText != null) {
                        var data = Ext.JSON.decode(response.responseText);
                        perTaskId = data.perTaskId;
                        Ext.Ajax.request({
                            url: APC.APPPLAT_SERVICE_URL_PREFIX + "OrbitFileManage/getTreeNodes.json",
                            method: 'GET',
                            async:false,
                            params: {
                                id: perTaskId,
                                targetNodeType:APC.NODETYPE_TASK
                            },
                            success: function (response) {
                                if (response.responseText != null) {
                                    var callbackJson = Ext.JSON.decode(response.responseText);
                                    me.getSolutionTree().getStore('SolutionTreeStore').setRootNode({
                                        id:callbackJson[0].id,
                                        name:callbackJson[0].name,
                                        leaf:callbackJson[0].leaf,
                                        nodeType:callbackJson[0].nodeType,
                                        parentId:callbackJson[0].parentId,
                                        rootId:callbackJson[0].rootId,
                                        isRoot:callbackJson[0].isRoot,
                                        nodeOrder:callbackJson[0].nodeOrder,
                                        dataId:callbackJson[0].dataId,
                                        nodeFilePath:callbackJson[0].nodeFilePath,
                        ...
                        me.getApplication().getController('AppPlat.controller.TreeController.TreeController').fileName = record[0].data.name;
                        me.getApplication().getController('AppPlat.controller.TreeController.TreeController').isFileSaved = true;
                        var listTree = me.getSolutionTree();
                        var rootNode = listTree.getRootNode();
                        ORBIT.saveManage.sceneRead(sessionStorage.getItem("userId") + "/" + APC.ORBIT_DESIGN + "/" + rootNode.internalId +
                            "/", rootNode.internalId);
                        ORBIT.fileName = record[0].data.name;
                    }
                    else {
...
//    },
    openVisibilityWindow:function (component, e){
        ...
        this.getButtongrouppanel().queryById('visibilityCheckboxGroup').setValue({
            planet: visibility
        });
    },
    //设置行星轨迹及其坐标系可见性的响应函数
    onVisbilityChg: function(checkboxgroup, newValue, oldValue, eOpts){
        ...
        if (visiblity['trace'] || visiblity['wind']) {
            ...
            if(ORBIT.tracedObject.id != "earth"){
                //切换到地球视角
                this.getButtongrouppanel().queryById('planetCombo').setValue('angelEarth');
//                //如果行星轨道为勾选状态，则显示轨道
                ...
                //切换到磁场坐标系
                this.getButtongrouppanel().queryById('earthAxesCombo').setValue('magnetic');
                ORBIT.zoom *= 20;
                if(visiblity['wind']){
                    ORBIT.zoom *= 2.5;
                }
            ...
            if(visiblity['wind']){
                ...
                //将行星轨迹复选框disable
                this.getButtongrouppanel().queryById('visibilityCheckboxGroup').queryById('planetaryTrack').disable();
                //将坐标系复选框disable
                this.getButtongrouppanel().queryById('visibilityCheckboxGroup').queryById('planetaryCoordinates').disable();
                //将地球磁场复选框disable
                this.getButtongrouppanel().queryById('visibilityCheckboxGroup').queryById('earthTrace').disable();
                //隐藏真太阳
                ...
                //将行星轨迹复选框enabl
                this.getButtongrouppanel().queryById('visibilityCheckboxGroup').queryById('planetaryTrack').enable();
                //将坐标系复选框enable
                this.getButtongrouppanel().queryById('visibilityCheckboxGroup').queryById('planetaryCoordinates').enable();
                //将地球磁场复选框enable
                this.getButtongrouppanel().queryById('visibilityCheckboxGroup').queryById('earthTrace').enable();
                //添加真太阳
