    ...
    Paging.prototype = {
        ...
        createDom: function() {
            var that = this,
                ...
                wrapLength = 0;
            totalPages > 3 ? wrapLength = 3 * liWidth : wrapLength = totalPages * liWidth;
            for (var i = 1; i <= that.options.totalPages; i++) {
                i != 1 ? ulDom += '<li>' + i + '</li>' : ulDom += '<li class="sel-page">' + i + '</li>';
            }
            that.options.jump ? jumpDom = '到第<input type="text" placeholder="1" class="jump-text" id="jumpText">页<button type="button" class="jump-button" id="jumpBtn">确定</button>' : jumpDom = '';
            content = '<p class="total-count">' +that.options.totalCount +'</p>'+
...
//              '<button type="button" id="lastPage" class="last-page">尾页</button>' +
        ...
        bindEvents: function() {
            ...
            function handles(pageIndex) {
                ...
                pageIndex == 1 ? firstPage.css('cursor', "not-allowed") : firstPage.css('cursor', "pointer");
                pageIndex == 1 ? prePage.attr('disabled', true) : prePage.attr('disabled', false);
                pageIndex == 1 ? prePage.css('cursor', "not-allowed") : prePage.css('cursor', "pointer");
                pageIndex == totalPages ? lastPage.attr('disabled', true) : lastPage.attr('disabled', false);
                pageIndex == totalPages ? lastPage.css('cursor', "not-allowed")  : lastPage.css('cursor', "pointer");
                pageIndex == totalPages ? nextPage.attr('disabled', true) : nextPage.attr('disabled', false);
                pageIndex == totalPages ? nextPage.css('cursor', "not-allowed") : nextPage.css('cursor', "pointer");
                that.options.callback(pageIndex);
            }
