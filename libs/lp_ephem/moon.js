...
//rm = r2r(0.347343 - 0.00014709391 * djd);
    var gm = r2r(0.374897 + 0.03629164709 * djd);
    var gm2 = 2 * gm;
    var gm3 = 3 * gm;
    var fm = r2r(0.259091 + 0.0367481952 * djd);
    var fm2 = 2 * fm;
    var em = r2r(0.827362 + 0.03386319198 * djd);
    var em2 = 2 * em;
    var em4 = 4 * em;
    var gs = r2r(0.993126 + 0.0027377785 * djd);
    var lv = r2r(0.505498 + 0.00445046867 * djd);
    var lm = r2r(0.606434 + 0.03660110129 * djd);
    var ls = r2r(0.779072 + 0.00273790931 * djd);
    var rm = r2r(0.347343 - 0.00014709391 * djd);
//
...
//l = l + 2 * (sin(4 * gm) + sin(em4 - gs) + sin(gm2 - em));
    var l = 22640 * Math.sin(gm) - 4586 * Math.sin(gm - em2) + 2370 * Math.sin(em2);
...
//- sin(gm - fm + em2 - gs));
    var b = 18461 * Math.sin(fm) + 1010 * Math.sin(gm + fm) + 1000 * Math.sin(gm - fm);
...
//plat = atr * (b + 2 * (sin(gm2 - fm - em2) + sin(gm3 - fm)));
    var plat = atr * (b + 2 * (Math.sin(gm2 - fm - em2) + Math.sin(gm3 - fm)));
//
//% obliquity of the ecliptic (radians)
//
//obliq = atr * (84428 - 47 * t + 9 * cos(rm));
    var obliq = atr * (84428 - 47 * t + 9 * Math.cos(rm));
//
...
//+ 0.00139 * cos(gm2 - em2 + gs);
    var r = 60.36298 - 3.27746 * Math.cos(gm) - .57994 * Math.cos(gm - em2);
...
//- 0.0011 * cos(gm2 - gs));
    var rmm = 6378.14 * (r - 0.00118 * Math.cos(gm - em4 + gs) - 0.00116 * Math.cos(gm2 + em2)- 0.0011 * Math.cos(gm2 - gs));
//
//% geocentric, equatorial right ascension and declination (radians)
//
//a = sin(plon) * cos(obliq) - tan(plat) * sin(obliq);
//b = cos(plon);
    var a = Math.sin(plon) * Math.cos(obliq) - Math.tan(plat) * Math.sin(obliq);
    var b = Math.cos(plon);
//rasc = atan3(a, b);
    var rasc = Math.atan3(a, b);
//
//decl = asin(sin(plat) * cos(obliq) + cos(plat) * sin(obliq) * sin(plon));
    var decl = Math.asin(Math.sin(plat) * Math.cos(obliq) + Math.cos(plat) * Math.sin(obliq) * Math.sin(plon));
//
...
//rmoon(3) = rmm * sin(decl);
    rmoon[0] = rmm * Math.cos(rasc) * Math.cos(decl);
    rmoon[1] = rmm * Math.sin(rasc) * Math.cos(decl);
    rmoon[2] = rmm * Math.sin(decl);
//
    return rmoon;//new Array(rasc, decl, rmoon);
}